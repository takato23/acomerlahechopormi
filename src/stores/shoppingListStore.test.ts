...
vi.mock('@/features/shopping-list/services/shoppingListService');
const mockedShoppingListService = vi.mocked(shoppingListService);
...
describe.skip('useShoppingListStore', () => {
  beforeEach(() => {
  ...
  it('updateItem should update an item in state (optimistic) and maintain sort order', async () => {
    ...
    expect(mockedShoppingListService.updateShoppingListItem).toHaveBeenCalledWith(
      mockItem1.id,
      updates,
    );
  });
