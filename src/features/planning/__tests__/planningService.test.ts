...
const mockSelect = mockFrom().select as Mock;
const mockUpsert = mockFrom().upsert as Mock;
const mockDelete = mockFrom().delete as Mock;
const mockEq = mockSelect().eq as Mock;
const mockOrder = mockSelect().order as Mock;
const mockLte = mockSelect().lte as Mock;
const mockGte = mockLte().gte as Mock;
const mockSingle = mockUpsert().select().single as Mock; // Asumiendo upsert -> select -> single
const mockThen = mockEq().then as Mock;
...
describe.skip('planningService', () => {
  beforeEach(() => {
    ...
    mockFrom.mockImplementation(() => ({
      select: mockSelect,
      insert: vi.fn(), // Añadir mocks faltantes si es necesario
      update: vi.fn(),
      delete: mockDelete,
      upsert: mockUpsert,
    }));
  ...
  describe('getPlannedMeals', () => {
    ...
    const mockMealsData = [
      { id: 'pm1', plan_date: '2023-02-03', meal_type: 'Almuerzo', user_id: mockUser.id },
    ];
    ...
    it('should throw error if fetch fails', async () => {
      ...
      await expect(getPlannedMeals(startDate, endDate)).rejects.toThrow(
        'No se pudieron cargar las comidas planificadas.',
      );
    });
  });
  ...
  describe('upsertPlannedMeal', () => {
    ...
    it('should throw error if upsert DB operation fails', async () => {
      ...
      await expect(upsertPlannedMeal(mealData)).rejects.toThrow(
        'No se pudo guardar la comida planificada.',
      );
    });
  });
  ...
  describe('deletePlannedMeal', () => {
    ...
    it('should return false if delete DB operation fails', async () => {
      ...
      mockEq.mockImplementationOnce(() => ({
        ...
        then: vi.fn((callback) => Promise.resolve({ error: new Error('Delete Fail') })),
      }));
