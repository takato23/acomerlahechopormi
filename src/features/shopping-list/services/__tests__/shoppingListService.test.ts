...
vi.mock('@/features/recipes/recipeService');
...
const mockedPlanningService = vi.mocked(planningService);
...
describe.skip('shoppingListService', () => {
  beforeEach(() => {
  ...
  describe('getShoppingListItems', () => {
    it('should fetch items ordered by purchased status and creation date', async () => {
      const mockListData = [
        { id: 'sl1', name: 'Item 1', is_purchased: false, created_at: '2023-01-01' },
      ];
      // Configurar cadena: select().eq().order(purchased).order(created).then()
      const mockThen = vi.fn().mockResolvedValueOnce({ data: mockListData, error: null });
      const mockCreatedOrder = vi.fn(() => ({ then: mockThen }));
      const mockPurchasedOrder = vi.fn(() => ({ order: mockCreatedOrder }));
      const mockEq = vi.fn(() => ({ order: mockPurchasedOrder }));
    ...
    it('should throw error if fetch fails', async () => {
      ...
      const mockThen = vi
        .fn()
        .mockResolvedValueOnce({ data: null, error: new Error('Fetch Fail') });
      const mockCreatedOrder = vi.fn(() => ({ then: mockThen }));
      const mockPurchasedOrder = vi.fn(() => ({ order: mockCreatedOrder }));
      const mockEq = vi.fn(() => ({ order: mockPurchasedOrder }));
      const mockSelect = vi.fn(() => ({ eq: mockEq }));
  ...
  describe('updateShoppingListItem', () => {
    ...
    it('should throw error if update fails', async () => {
      ...
      const mockUpdate = vi.fn(() => ({ eq: mockEq }));
      mockFrom.mockImplementationOnce(() => ({ update: mockUpdate }));
      await expect(updateShoppingListItem(itemId, updates)).rejects.toThrow(
  ...
  describe('deleteShoppingListItem', () => {
    ...
    it('should throw error if delete fails', async () => {
      ...
      const mockDelete = vi.fn(() => ({ eq: mockEq }));
      mockFrom.mockImplementationOnce(() => ({ delete: mockDelete }));
      await expect(deleteShoppingListItem('sl-delete-fail')).rejects.toThrow(
  ...
  describe('clearPurchasedItems', () => {
    it('should delete items where user_id matches and is_purchased is true', async () => {
      ...
      const mockThen = vi.fn().mockResolvedValueOnce({ error: null });
      const mockPurchasedEq = vi.fn(() => ({ then: mockThen }));
      const mockUserEq = vi.fn(() => ({ eq: mockPurchasedEq }));
      const mockDelete = vi.fn(() => ({ eq: mockUserEq }));
      mockFrom.mockImplementationOnce(() => ({ delete: mockDelete }));
    ...
    it('should throw error if delete fails', async () => {
      const mockThen = vi.fn().mockResolvedValueOnce({ error: new Error('Clear Fail') });
      const mockPurchasedEq = vi.fn(() => ({ then: mockThen }));
      const mockUserEq = vi.fn(() => ({ eq: mockPurchasedEq }));
      const mockDelete = vi.fn(() => ({ eq: mockUserEq }));
      mockFrom.mockImplementationOnce(() => ({ delete: mockDelete }));
      await expect(clearPurchasedItems()).rejects.toThrow(
        'No se pudieron limpiar los ítems comprados.',
      );
    });
  });
  ...
  describe('getFrequentItems', () => {
    it('should return frequently added item names', async () => {
      ...
      const mockThen = vi.fn().mockResolvedValueOnce({ data: mockItems, error: null });
    ...
    it('should return empty array if fetch fails', async () => {
      const mockThen = vi.fn().mockResolvedValueOnce({ data: null, error: new Error('Freq Fail') });
      const mockLimit = vi.fn(() => ({ then: mockThen }));
      const mockEq = vi.fn(() => ({ limit: mockLimit }));
      const mockSelect = vi.fn(() => ({ eq: mockEq }));
      mockFrom.mockImplementationOnce(() => ({ select: mockSelect }));
  ...
  describe('generateShoppingList', () => {
    ...
    it('should not add item if pantry stock is sufficient', async () => {
      const mockPantrySufficient: PantryItem[] = [
        ...
        },
        {
          ...
          created_at: mockDate,
          category_id: null,
          ingredient_id: mockIngredientIdSauce,
    ...
    it('should handle items with null quantity in recipe', async () => {
      const mockRecipeNullQty: Recipe = {
        ...
        created_at: mockDate,
        recipe_ingredients: [
          { id: 'i3', recipe_id: 'r2', name: 'Lettuce', quantity: null, unit: null },
        ],
      };
    ...
    it('should add custom meal names directly', async () => {
      const mockPlannedCustom: PlannedMeal[] = [
        {
          id: 'pm3',
          plan_date: '2023-01-05',
          meal_type: 'Desayuno',
          custom_meal_name: 'Yogurt con Fruta',
          recipe_id: null,
          user_id: mockUser.id,
          created_at: mockDate,
        },
    ...
    it('should throw error if user is not authenticated', async () => {
      mockGetUser.mockResolvedValueOnce({ data: { user: null }, error: null });
      await expect(generateShoppingList(startDate, endDate)).rejects.toThrow(
