...
} from './services/buscaPreciosService'; // Recuperado y añadido SearchProductsResult
...
import {
  ...
  AccordionItem,
  AccordionTrigger,
} from '@/components/ui/accordion'; // Recuperado
...
import {
  getCategoryMetadata,
  getDisplayCategory,
  mapLabelToCategoryKey,
} from './utils/categorization'; // Importar utilidades de categorías
...
type GroupedShoppingListItems = {
  [category: string]: Database['public']['Tables']['shopping_list_items']['Row'][];
};
...
export function ShoppingListPage() {
  ...
  const filteredListItems = useMemo(() => {
    ...
    return listItems.filter((item) => {
      ...
      return (
        lowerCaseItemName.includes(lowerCaseSearchTerm) ||
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      // Solo trackear si la búsqueda está activa
  ...
  const mapContent = shouldShowFullMap ? (
    <ShoppingMap onToggleFavorite={handleToggleFavoriteStore} favoriteStoreIds={favoriteStoreIds} />
  ) : (
    <FavoriteStoresInfo
  ...
  const handleSearchItemPrice = useCallback(
    async (item: Database['public']['Tables']['shopping_list_items']['Row']) => {
      ...
      try {
        ...
        if (!result.error) {
          ...
          console.error(
            `Error buscando precios para ${item.ingredient_name}:`,
  ...
  const renderShoppingListContent = () => (
    <div className="flex flex-col h-full flex-grow">
      <div className="p-4 pb-2 flex-shrink-0 flex flex-col gap-3">
        {' '}
        {/* Reducir padding inferior, evitar encogimiento */}
        <ShoppingListToolbar
          items={listItems}
      ...
      <Tabs defaultValue="lista" className="flex-grow flex flex-col overflow-hidden px-4 pb-4">
        <TabsList className="mb-2 flex-shrink-0">
          {' '}
          {/* Evitar encogimiento de la lista de tabs */}
        ...
        <TabsContent value="lista" className="flex-grow flex flex-col overflow-hidden p-0 m-0">
          <Card className="flex-grow flex flex-col overflow-hidden bg-card">
            {' '}
            {/* Usar estilos base de Card Y FORZAR bg-card */}
            ...
            <CardContent className="p-0 flex-grow overflow-y-auto">
              {isLoading ? (
                ...
                <Alert variant="destructive" className="m-4">
                  <AlertDescription>
                    {typeof error === 'string' ? error : 'Error al cargar la lista'}
                  </AlertDescription>
                </Alert>
              ) : listItems.length === 0 ? (
                      ...
                      {itemsInCategory.map((item) => (
                        <li
                          ...
                          className={cn(
                            ...
                            item.is_checked ? 'opacity-60 bg-muted/50' : 'hover:bg-muted/50',
                          )}
                        >
  ...
  return (
    ...
    >
      <PageSection padded={false} className="overflow-hidden min-h-[60vh]" contentClassName="p-0">
        <ResponsiveLayout shoppingList={renderShoppingListContent()} map={mapContent} />
      </PageSection>
    </PageLayout>
  );
}
