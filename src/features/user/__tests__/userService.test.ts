...
const mockUpdate = mockFrom().update as Mock;
const mockEq = mockSelect().eq as Mock;
const mockSingle = mockEq().single as Mock;
const mockThen = mockEq().then as Mock;
const mockStorageFrom = supabase.storage.from as Mock;
const mockUpload = mockStorageFrom().upload as Mock;
const mockGetPublicUrl = mockStorageFrom().getPublicUrl as Mock;
...
describe.skip('userService', () => {
  beforeEach(() => {
    ...
    mockEq.mockImplementation(() => ({
      ...
      single: mockSingle,
      eq: mockEq,
      order: vi.fn(), // Añadir order si es necesario
    }));
  ...
  describe('getUserProfile', () => {
    ...
    it('should return basic user data if profile fetch fails (not PGRST116)', async () => {
      ...
      await expect(getUserProfile(mockUser.id)).resolves.toEqual(
      ...
      expect(console.warn).toHaveBeenCalledWith(
        'Error fetching profile (but not PGRST116):',
        'DB Error',
      );
    });
  ...
  describe('updateUserProfile', () => {
    it('should throw error if user is not authenticated', async () => {
      mockGetUser.mockResolvedValueOnce({ data: { user: null }, error: null });
      await expect(updateUserProfile(mockUser.id, { username: 'new' })).rejects.toThrow(
    ...
    it('should return false if supabase.update fails', async () => {
      ...
      mockEq.mockImplementationOnce(() => ({
        // update().eq()
        then: vi.fn((callback) => Promise.resolve(callback({ error: new Error('Update Failed') }))),
      }));
  ...
  describe('uploadAvatar', () => {
    ...
    const mockPublicUrl =
      'https://supabase.co/storage/v1/object/public/avatars/user-123-timestamp.png';
    ...
    it('should return null if updateUserProfile fails', async () => {
      ...
      mockEq.mockImplementationOnce(() => ({
        // update().eq()
        then: vi.fn((callback) =>
          Promise.resolve(callback({ error: new Error('Profile Update Failed') })),
        ),
      }));

      await expect(uploadAvatar(mockFile)).resolves.toBeNull();
      expect(console.warn).toHaveBeenCalledWith(
        'Avatar uploaded but failed to update profile URL.',
      );
      expect(console.error).toHaveBeenCalledWith('Error updating profile:', expect.any(Error));
    });
  });
